...
bool Binary_Value_Write_Property(BACNET_WRITE_PROPERTY_DATA *wp_data)
{
    ...
    struct object_data *pObject;
    BACNET_BINARY_PV write_value = BINARY_INACTIVE;
    ...
    if (len < 0) {
        ...
        debug_printf(
            "BV Write decode failed: inst=%lu prop=%u len=%u\n",
            ...
            switch (value.tag) {
                case BACNET_APPLICATION_TAG_ENUMERATED:
                    if (value.type.Enumerated <= MAX_BINARY_PV) {
                        write_value = (BACNET_BINARY_PV)value.type.Enumerated;
                    } else {
                        status = false;
                        wp_data->error_class = ERROR_CLASS_PROPERTY;
                        wp_data->error_code = ERROR_CODE_VALUE_OUT_OF_RANGE;
                ...
                case BACNET_APPLICATION_TAG_BOOLEAN:
                    write_value = value.type.Boolean ? BINARY_ACTIVE : BINARY_INACTIVE;
                    break;
                case BACNET_APPLICATION_TAG_UNSIGNED_INT:
                    if (value.type.Unsigned_Int <= 1) {
                        write_value = value.type.Unsigned_Int ? BINARY_ACTIVE : BINARY_INACTIVE;
                    } else {
                        status = false;
                        wp_data->error_class = ERROR_CLASS_PROPERTY;
                        wp_data->error_code = ERROR_CODE_VALUE_OUT_OF_RANGE;
                ...
                case BACNET_APPLICATION_TAG_SIGNED_INT:
                    if (value.type.Signed_Int == 0) {
                        write_value = BINARY_INACTIVE;
                    } else if (value.type.Signed_Int == 1) {
                        write_value = BINARY_ACTIVE;
                    } else {
                        status = false;
                        wp_data->error_class = ERROR_CLASS_PROPERTY;
                        wp_data->error_code = ERROR_CODE_VALUE_OUT_OF_RANGE;
