...
extern void bacnet_nvs_save_av_name(uint32_t instance, const char *name, uint16_t length);
extern void bacnet_nvs_save_av_desc(uint32_t instance, const char *desc, uint16_t length);
...
bool Analog_Value_Write_Property(BACNET_WRITE_PROPERTY_DATA *wp_data)
{
    ...
    if (len < 0) {
        /* try context-tagged decode for clients that send context tags */
    ...
    if (len < 0) {
        ...
        debug_printf(
            "AV Write decode failed: inst=%lu prop=%u len=%u\n",
            (unsigned long)wp_data->object_instance,
            (unsigned)wp_data->object_property,
            (unsigned)wp_data->application_data_len);
        return false;
    }
        ...
        case PROP_PRESENT_VALUE:
            ...
            switch (value.tag) {
                case BACNET_APPLICATION_TAG_REAL:
                    write_value = value.type.Real;
                    break;
                case BACNET_APPLICATION_TAG_DOUBLE:
                    write_value = (float)value.type.Double;
                    break;
                case BACNET_APPLICATION_TAG_UNSIGNED_INT:
                    write_value = (float)value.type.Unsigned_Int;
                    break;
                case BACNET_APPLICATION_TAG_SIGNED_INT:
                    write_value = (float)value.type.Signed_Int;
                    break;
                case BACNET_APPLICATION_TAG_ENUMERATED:
                    write_value = (float)value.type.Enumerated;
                    break;
                default:
                    status = false;
                    wp_data->error_class = ERROR_CLASS_PROPERTY;
        ...
        case PROP_UNITS:
            ...
            if (status) {
                if (value.type.Enumerated <= UINT16_MAX) {
                    ...
                    bacnet_nvs_save_av_units(wp_data->object_instance,
